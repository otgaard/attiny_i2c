...
#include <avr/pgmspace.h> 
 ...
 const uint16_t sin_tbl[256] PROGMEM = {
...
void sine_wave(uint16_t* value) {
	start_i2c();
	transmit_i2c(MCP4725_ADDR_WRITE);
	uint16_t val = pgm_read_word(&sin_tbl[curr_value]);
	transmit_i2c(0x40);
	transmit_i2c(val >> 4);
	transmit_i2c((val & 0x0F) << 4);
	stop_i2c();
	*value = val;
	curr_value++;
}